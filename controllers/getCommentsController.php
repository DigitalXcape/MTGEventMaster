...
{
    private $logger;
    private $baseApiUrl = 'http://localhost/MTGEventAPI/endpoints/getComments.php';
    ...
    //get all the comments of an event
    public function getComments($eventId)
    {
        ...
        if (json_last_error() === JSON_ERROR_NONE && isset($data['success']) && $data['success']) {
            $this->logger->log("API response received successfully: " . json_encode($data['comments']));
